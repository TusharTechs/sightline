...
| Missing audio hardware | `/proc/asound/cards` → `virtio-snd - VirtIO SoundCard`; `/proc/asound/pcm` → `VirtIO PCM 0 : playback 4 : capture 2`. Further confirmed: the boot animation plays audio audibly on the host, and `animationservice` logs `Audio Focus 1 is granted` / `initial audio frame presented to hardware` on every boot. Device audio works. |
